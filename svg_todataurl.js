...
SVGElement.prototype.toDataURL = function(type, keepNonSafe, keepOutsideViewport) {
		...
		if (keepNonSafe) debug("keepNonSafe not supported!")
		if (keepOutsideViewport) debug("keepOutsideViewport not supported!")
		...
		switch (type) {
			...
			case "image/png":
				// SECURITY_ERR: return SVGtoDataURL.exportPNG(this);
				if (!canvg) {
					alert("Sorry! You need canvg for PNG exporting.\n" +
